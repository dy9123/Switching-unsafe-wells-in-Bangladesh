...
boot.fn(wells,1:dim(wells)[1]) # We used bootstrap to get the coefficients of logistic regression
```

```{r}
set.seed(1)
boot(wells,boot.fn,100) #  We used bootstrap to get the error
```{r}
glm.fit.final = glm(switch~sqrt(arsenic)+unsafe+sqrt(distance)+I(x^9)+I(y^4)+I(education^5)+
                    arsenic:x+unsafe:distance+unsafe:x+unsafe:y+distance:education+x:y
              ,data=wells,family=binomial)
summary(glm.fit.final)$coef #
```